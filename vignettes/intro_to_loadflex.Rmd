...
USGS gaging station (http://waterdata.usgs.gov/usa/nwis/uv?site_no=01073500).
...
The `loadflex` process for aggregation from point predictions to monthly predictions has changed since we published our 2015 manuscript, so `aggregateSolute` from that manuscript is no longer offered as a function in loadflex. Instead, you can use the `agg.by` argument to `predictSolute` to generate predictions at the time interval of interest. You can aggregate for mean concentration or flux rate, and for months, water years, calendar years, or other time intervals. `n` reports the number of observations going into the aggregated estimate in each row. Some examples:
```{r}
