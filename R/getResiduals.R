...
getResiduals <- function(load.model, flux.or.conc=c("flux","conc"),
  ...
  preds <- predictSolute(
    load.model=load.model, flux.or.conc=flux.or.conc, newdata=newdata,
    date=FALSE, count=FALSE, attach.units=FALSE)
  obs <- observeSolute(
    data=newdata, flux.or.conc=flux.or.conc,
    metadata=getMetadata(load.model), attach.units=FALSE)
  if(use.log) {
    preds <- log(preds)
    obs <- log(obs)
  }
